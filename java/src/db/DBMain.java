...
public class DBMain {
	...
	public static void main(String[] args) {
		...
		String pr_st_num = scan.next();
	
		if(dbDao.insertStudent(st_num, st_name, pr_st_num) == 1 ) {
	...
	public static Connection connect() {
		...
		try {
			...
			//Class.forName(driverName);에서 예외 발생
			System.out.println(driverName +"클래스를 찾을 수 없습니다.");
		
		}catch(SQLException e) {
			//con = DriverManager.getConnection (url,user,pw)에서 예외 발생
			// 1없는 DB에 접근할 떄 2. id/pw이 틀렸을 때 
