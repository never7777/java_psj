...
public class Ex6_Program_Student {
	/*학생 정보(이름, 학년, 반, 번호, 국어, 영어, 수학 성적)을 관리하는 프로그램을 작성하세요.
	 * 1. 기능 정리 => 주석
	 *  - 학생정보 입력
	...
	public static void main(String[] args) {
		...
		for(;menu!=5;) {
			...
			if(menu==1) {
				...
				//index 번지에 저장
				std[index] = new EX6_Student(name, grade, classRoom, number, kor, eng, math);
				//index를 1증가
				index++;
				
			}else if(menu==2) {
				//반복문을 이용하여 0번지부터 index명 만큼 학생 정보를 출력
				...
				classRoom = scan.nextInt();
				number = scan.nextInt();
				//반복문을 통해 0번지 부터 index명 만큼 학생 정보를 하나씩 가져와서 
				for(int i = 0;i<index;i++) {
					//입력받은 학년, 반, 번호와 일치하는 학생이 있으면 
					if(std[i].equal(grade,classRoom,number)) {
						//이름, 국어, 영어, 수학 성적을 입력 받은 후 
						System.out.println("학생 성적을 입력하세요(이름, 국어, 영어, 수학순) : ");
						name = scan.next();
						kor = scan.nextInt();
						eng = scan.nextInt();
						math = scan.nextInt();
						//학생 정보를 수정하고, 반복문을 종료
						std[i].studentChange(name, kor, eng, math);
						break;
					}
				...
				for(int i = 0;i<index;i++) {
					//일치하는 학생 정보가 있으면 delIndex번지를 기억하고 반복문을 종료
					if(std[i].equal(grade,classRoom,number)) {
						delIndex = i;
						break;
					}
	...
	/* 기능 : 메뉴를 출력하고 메뉴룰 입력하면 입력한 메뉴를 돌려주는 메소드
