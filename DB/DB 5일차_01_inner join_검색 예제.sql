...
select distinct usertbl.userID, prodName from usertbl
	join buytbl
    on usertbl.userId = buytbl.userId
    where prodName = '운동화';
...
select 학생.이름, 교수.이름 from 학생
join 교수
on 학생.교번 = 교수.교번; 

-- 고길동 학생이 수강하는 강좌코드 (강좌코드)를 조회
...
-- 나영석 교수가 강의하는 강의를 듣는 학번을 조회(중복제거)
select distinct 이름, 학번 from 강좌
	join 교수 on 교수.교번 = 강좌.교번
    join 수강 on 수강.강좌코드 = 강좌.강좌코드
	where 이름 = '나영석';
    

-- 나영석 교수가 강의하는 강의를 듣는 이름을 조회(중복제거)
...
-- 나영석 교수의 지도학생을 이름을 조회
select 교수.이름 as '교수이름', 학생.이름 as '학생이름' from 학생
	join 교수 on 교수.교번 = 학생.교번
	where 교수.이름 = '나영석';

-- 두테이블(테이블 A, 테이블 B)을 연결하는데 on 없이 하는 경우
