...
-- 오류 발생했을 시, 하단 action output에서 팝업메뉴(우클릭) 띄우고, copy response 클릭하면 오류 message 복사 가능  

/* 1학년 1반 1번 학생의 1학년 1학기 국어 성적은 중간 100, 기말 90, 수행 80 이다.
   이 정보를 테이블에 추가하는 쿼리를 작성해보세요
   1. 학생 테이블에 1학년 1반 1번 학생이 있는지 확인(있다. 핵생번호가 1번. 없으면 =>추가)
   2. 성적 테이블에 1학년 1학기 국어 성적을 추가
   3. 보유 테이블에 학생번호가 1번이고, 성적번호가 1이고, 중간 100, 기말, 90, 수행 80을 추가
*/
insert into 성적(과목명, 학년, 학기) values ('국어', 1, 1);
insert into 보유(학생번호, 성적번호, 중간, 기말, 수행) values (1, 1, 100, 90, 80);

/* 1학년 1반 2번 학생의 1학년 1학기 국어 성적은 중간 80, 기말 85, 수행 89 이다.
   이 정보를 테이블에 추가하는 쿼리를 작성해보세요
   1. 학생 테이블에 1학년 1반 1번 학생이 있는지 확인(있다. 학생번호가 2번. 없으면 =>추가)
   2. 성적 테이블에 1학년 1학기 국어 성적을 추가 (있다. 성적번호가 1번. 없으면 =>추가)
   3. 보유 테이블에 학생번호가 2번이고, 성적번호가 1이고, 중간 80, 기말 85, 수행 89을 추가
*/
insert into 보유(학생번호, 성적번호, 중간, 기말, 수행) values (2, 1, 80, 85, 89);

/* 1학년 1반 3번 이순신학생의 1학년 1학기 수학 성적은 중간 870, 기말 90, 수행 100 이다.
   이 정보를 테이블에 추가하는 쿼리를 작성해보세요
   1. 학생 테이블에 1학년 1반 1번 학생이 있는지 확인(있다. 학생번호가 2번. 없으면 =>추가)
   2. 성적 테이블에 1학년 1학기 국어 성적을 추가 (있다. 성적번호가 1번. 없으면 =>추가)
   3. 보유 테이블에 학생번호가 2번이고, 성적번호가 1이고, 중간 80, 기말 85, 수행 89을 추가
*/
insert into 학생(이름, 학년, 반, 번호) values ('이순신', 1,1,3);
insert into 성적(학년, 학기, 과목명) values (1,1,'수학');
insert into 보유(학생번호, 성적번호, 중간, 기말, 수행) values (3, 2, 70, 90, 100);
