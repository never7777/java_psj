...
-- bd_num에 auto_increment로 변경하는 퀴리문
...
insert into user(us_id, us_pw, us_name, us_phone, us_authority)
	values ('abcd1234','abcd1234','홍길동','020-1234-5678',1),
	('qwer1234','qwer1234','임꺽정','030-1234-5678',1);
...
insert into category (ca_name,ca_w_authority, ca_r_authority,ca_secret_pos)
	values ('공지사항',10,1,0),('일반',1,0,0),('문의사항',1,1,1);


-- abcd1234 회원의 일반 게시글에 제목: 안녕하세요  내용 : 만나서 반갑습니다. 라는 글을 등록했다
-- 최고관리자가 공지사항에 제목: 공지사항 1 내용: 공지사항입니다라는 글을 등록했다.
-- qwer1234회원의 문의 게시글에 제목: 문의합니다 내용: 비번을 바꾸고 싶엉 라는 비밀글로 등록했다.
...
-- qwer1234회원이 5번 게시글에 답글은 남김. 제목 : 그래도 안돼요.내용 : 비번찾기를 이용해도 안됩니다.
insert into board(bd_ca_name, bd_title, bd_contents, bd_us_id, bd_secret, bd_ori_num, bd_depth)
	select '문의사항', '그래도 안돼요', '비번찾기 이용해도 안됩니다.','qwer1234', 0,
		bd_ori_num, bd_depth+1 from board
        where bd_num=5;

-- 3번 게시글을 조회 했을 때 필요한 쿼리=> 조회수 증가
update board set bd_views = bd_views+1
where bd_num = 3 ;

-- 추천/비추천을 했다=> 해당 게시글/댓글에 사용자가 처음 추선/비추하는 경우=> 등록, 아닌 경우 => 수정(삭제)

 -- 1번 게시글에 qwer1234 회원이 추천했을 때 쿼리문 (게시글을 확인하고, 추천 버튼을 클릭했다고 가정)
 select * from likes;
 insert into likes (li_us_id, li_state, li_target, li_target_num)
 values ('qwer1234',1,'board',1);
 
 -- qwer1234회원이 1번 게시글에 추천을 한번 더 눌러 추천을 취소했을 때 쿼리문
...
insert into board(bd_ca_name, bd_us_id, bd_title, bd_contents, bd_us_idm, bd_secret, bd_ori_num) 
	select '일반', 'abcd123','제목입니다2','내용입니다', 0, ifnull(max(bd_num),0)+1 from borad;
