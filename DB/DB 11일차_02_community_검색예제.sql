...
-- 1번 게시글의  추천수를 조회
select count(li_state)as 추천수 from likes
	where li_target_num = 1 and li_target='board'and li_state = 1;   

select count(*) as 1번게시글추천수 from likes
	where li_target_num = 1 and li_target='board'and li_state = 1;   

-- 1번 게시글의 비추천수를 조회
select count(li_state) as 비추천수 from likes
	where li_target_num = 1 and li_target='board'and li_state < 0;   
