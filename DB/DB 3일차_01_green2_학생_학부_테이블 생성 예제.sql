...
-- create schema green2;
create database if not exists green2;
-- if not exists(만약 존재하지 않으면) 
...
-- 학부 테이블 컴퓨터 공학부를 추가(그린대학교 1관 401호)
...
-- 학생 테이블에 컴퓨터 공학부 홍길동 학생 정보를 추가하는 쿼리문을 작성하세요.
insert into 학생  values (1,'홍길동', '컴퓨터공학부');
-- insert into 학생 (학생번호, 이름, 학부) values (1,'홍길동', '컴퓨터공학부');


-- 컴퓨터공학부 학부위치가 그린대학교 2관 101호로 이동을 함
-- update에서 set 있는  =는 값을 수정하는 역할, where에 있는 =은 값을 비교하는 역할
update 학부
	set 
		학부위치 = '그린대학교 2관 101호'
    where 
